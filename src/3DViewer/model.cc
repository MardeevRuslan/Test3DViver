...
int Model::get_polygon_count() const { return my_object_->polygon_count_; }
int Model::get_vertex_count() const { return my_object_->vertex_count_; }
void Model::set_transform(std::string key, float value) {
  transform_[key] = value;
}
float Model::get_transform(std::string key) const { return transform_.at(key); }

std::vector<int> Model::get_poligon(int i) const {
...
GLuint *Model::get_indices() const {
...
void Model::ModelTransform() {
  for (size_t i = 0; i < vector_koordinate_.size() - 2; i += 3) {
    ...
    vector_koordinate_[i + 1] =
        coordinates_[1] * transform_["scale"] +
        (transform_["translateY"] / kSmoothnessOfMovement);
    vector_koordinate_[i + 2] =
        coordinates_[2] * transform_["scale"] +
